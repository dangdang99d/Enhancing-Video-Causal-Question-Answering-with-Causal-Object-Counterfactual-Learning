...
class Language_Retrieval_Block(nn.Module):
    def __init__(self,
        ...
        self.inject_lang_summary = inject_lang_summary

        if(not self.do_time_specific_proj):
            ...
            self.lang_mlp = nn.Sequential(linear(dim, dim),
                                          get_activation_layer(lang_block_nonlin),
                                          nn.Dropout(lang_block_mlp_drop))
            self.time_specific_projs = nn.ModuleList([linear(dim, dim) for i in range(num_time_steps)])

        if(self.inject_lang_summary):
            self.lang_summary_proj = linear(dim, dim)
            self.lang_gate = nn.Sequential(linear(dim * 2, dim), nn.Sigmoid())
    ...
    def forward(
        ...
        if(not self.do_time_specific_proj):
            op_query = self.lang_mlp(query)
        else:
            op_query = self.time_specific_projs[step](self.lang_mlp(query))

        if(self.inject_lang_summary and orig_lang_summary is not None):
            lang_proj = self.lang_summary_proj(orig_lang_summary)
            if lang_proj.ndim == 2:
                lang_proj = lang_proj.unsqueeze(1).expand_as(op_query)
            gate = self.lang_gate(torch.cat([op_query, lang_proj], dim=-1))
            modulation = op_query + gate * lang_proj
        else:
            modulation = op_query

        lang_tokens_rep_prod = modulation.unsqueeze(2) * lang_tokens_rep.unsqueeze(1)
...
class Recurrent_Memory_Attention_Module(nn.Module):
    def __init__(self, 
        ...
        self.lang_retrieval_block = Language_Retrieval_Block(dim,
                                                             ...
                                                             do_time_specific_proj=cfg.IPRM.DO_TIME_STEP_SPECIFIC_PROJ,
                                                             num_time_steps=self.max_computation_steps,
                                                             inject_lang_summary=cfg.IPRM.MEMORY_OP_STATE_INIT in ['lang_projs'],
                                                            )
        ...
        elif(cfg.IPRM.MEMORY_RES_STATE_INIT in ['lang_projs']):
            self.res_init_projs = nn.ModuleList([linear(dim, dim) for i in range(self.num_memory_tokens)])
        else:
            None
        ...
        # Causal Gate: op_state-conditioned soft gate on visual tokens
        self.use_causal_gate = getattr(cfg.IPRM, 'USE_CAUSAL_GATE', False)
        if self.use_causal_gate:
            half_dim = dim // 2
            self.causal_gate_op_proj  = linear(dim, half_dim)
            self.causal_gate_vis_proj = linear(dim, half_dim)
            self.causal_gate_score    = linear(half_dim, 1)
    ...
    def _init_memory(self, 
        ...
        elif(self.cfg.IPRM.MEMORY_RES_STATE_INIT in ['lang_projs']):
            assert(lang_cuml is not None)
            res_states = torch.stack([res_proj(lang_cuml) for res_proj in self.res_init_projs], dim=0)
            res_states = res_states.transpose(0,1) #Nm x B xD -> BxNmxD
        else:
    ...
    def forward(
        ...
        for i in range(self.max_computation_steps):
            retrieved_lang, text_attention = self.lang_retrieval_block(
                ...
                orig_lang_summary = lang_summary_rep,
            )
            
            text_attentions.append(text_attention)

            # Causal Gate: op_state로 각 visual token에 soft importance weight 적용
            if self.use_causal_gate:
                # op_state: BxNmxD -> mean pool -> BxD
                op_pooled = op_state.mean(dim=1)                          # BxD
                # vis_tokens_rep: BxDxNv -> BxNvxD
                vis_t = vis_tokens_rep.transpose(-1, -2)                  # BxNvxD
                op_proj  = self.causal_gate_op_proj(op_pooled).unsqueeze(1)  # Bx1x(D/2)
                vis_proj = self.causal_gate_vis_proj(vis_t)               # BxNvx(D/2)
                gate = torch.sigmoid(
                    self.causal_gate_score(torch.tanh(op_proj + vis_proj))
                ).squeeze(-1)                                             # BxNv
                vis_input = vis_tokens_rep * gate.unsqueeze(1)            # BxDxNv
            else:
                vis_input = vis_tokens_rep

            retrieved_vis, attention = self.vis_retrieval_block(res_state, vis_input, retrieved_lang, op_state, vis_token_mask=vis_token_mask)
